...
const LoanApplicationForm = () => {
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/categorize-transactions/', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
            setCategorizedResults(response.data.categorized_transactions);
        } catch (error) {
            console.error('Categorization failed:', error);
            alert('Categorization failed. See console for details.');
        }
    };
    ...
    return (
        <div className="loan-form-container">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="loan-form">
                ...
                <fieldset>
                    <legend>Upload Bank Statement</legend>
                    <div className="form-group">
                ...
                {categorizedResults && (
                    <div className="categorized-results">
                        <h4>Categorized Transactions</h4>
                        <ul>
                            {categorizedResults.map((txn, index) => (
                                <li key={index}>
                                    {txn.description} - ${txn.amount} - <strong>{txn.category}</strong>
                                </li>
                            ))}
                        </ul>
                    </div>
